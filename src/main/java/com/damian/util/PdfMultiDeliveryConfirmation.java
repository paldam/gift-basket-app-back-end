...
public class PdfMultiDeliveryConfirmation {

    static Integer orderTypeId;
    static Order order;

    public static ByteArrayInputStream generateMultiPdf(java.util.List<Order> orderToPrint) throws IOException {
        ...
        orderToPrint.forEach(order -> {
            try {
                ...
                PdfPCell cell18 = new PdfPCell(new Phrase(order.getCustomer().getName(), font3));
                cell18.setColspan(4);
                ...
                //row10
                PdfPCell cell27 = new PdfPCell(new Phrase("Potwierdzam odbiór zestawów świątecznych zgodnie z " +
                    "zamówieniem ", font15));
    ...
    private static String getAdditionalInformtionMassage(Order order) {
        ...
        if (Arrays.asList(5, 6, 7).contains(orderTypeId)) {
            additionalInformationTmp.append("Pobranie ").append((double) order.getCod() / 100).append(" zł | ").append((order.getAdditionalInformation() == null) ? "" : order.getAdditionalInformation());
        } else {
            additionalInformationTmp.append((order.getAdditionalInformation() == null) ? "" :
