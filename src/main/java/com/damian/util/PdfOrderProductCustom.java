...
public class PdfOrderProductCustom {
    ...
    public static ByteArrayInputStream generateBasketsProductsCustomListPdf(java.util.List<OrderItemsDto> orderItemsDto, Order order) throws IOException {
        ...
        try {
            ...
            stringBuilder.append("Zamówienie: ").append(order.getOrderFvNumber()).append(" | Firma: ").append(order.getCustomer().getCompany().getCompanyName()).append(" | Osoba: ").append(order.getCustomer().getName()).append(" | Data wysyłki: ").append(orderDateString).append(" | Produkcja: ");
            ...
            orderItemsDto.forEach(orderItems -> {
                if (orderItems.getAdded() == true) {
                    ...
                    PdfPCell cell = new PdfPCell(new Phrase("Kosz: " + orderItems.getBasket().getBasketName() + "    " +
                        "    Ilość: " + orderItems.getQuantity(), font4));
